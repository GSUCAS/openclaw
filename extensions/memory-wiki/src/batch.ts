// Memory Wiki plugin module implements bounded machine-oriented batch operations.
import fs from "node:fs/promises";
import path from "node:path";
import { applyMemoryWikiMutation } from "./apply.js";
import { compileMemoryWikiVault, type CompileMemoryWikiResult } from "./compile.js";
import type { ResolvedMemoryWikiConfig } from "./config.js";
import { ingestMemoryWikiSource, type IngestMemoryWikiEvidence } from "./ingest.js";
import {
...
async function normalizeApplyOperations(input: JsonRecord): Promise<ApplyBatchOperation[]> {
  ...
  for (const raw of input.operations) {
    ...
    const resolvedBody = body ?? (await fs.readFile(path.resolve(bodyFile as string), "utf8"));
    if (Buffer.byteLength(resolvedBody) > BATCH_SYNTHESIS_MAX_BYTES) {
  ...
  for (const operation of operations) {
    ...
    const resolvedSourceIds = [
      ...
      ...operation.sourceRefs.map((ref) => {
        ...
        if (!sourceId) {
          throw new Error(`upsert-synthesis sourceRef must name an earlier ingest-source: ${ref}`);
        }
        return sourceId;
      }),
    ];
